...
body.push(P([T('Testing across '), bold('193 logs from two players (builds 4.7.0–4.8.0)'), T(' established that the game records only the kills that '), bold('involve the running player'), T(' (your kills and your deaths) — not third-party kills. This is now '), bold('verified end-to-end against a member’s real logs'), T(' (417 kills + 16 ship destructions parsed correctly). '), bold('Player-downs (incapacitation)'), T(' are also detected. A player’s own log is self-reported, so the register uses '), bold('officer validation'), T(' as the authority for completion, with log activity attached as supporting evidence. The same model applies to '), bold('out-of-game missions and fleet actions'), T(', which have no log signal.')]));
