...
test('normalizeCommodities keeps visible items, flags illegal, sorts by name', () => {
  const raw = [
    ...
    { name: 'Ghost', is_visible: 0 },
    { code: 'NONAME', is_visible: 1 }
  ];
  const out = normalizeCommodities(raw);
  assert.deepStrictEqual(out.map((c) => c.name), ['Tungsten', 'Widow']);
...
test('bodyOfTerminal maps Pyro system to Pyro and Stanton planets to their bucket', () => {
  ...
  assert.strictEqual(bodyOfTerminal({ star_system_name: 'Nyx', planet_name: null }), null);
});
...
test('normalizeLocations emits distinct PLACE names (station/city/outpost) with body', () => {
  const raw = [
    ...
    { star_system_name: 'Stanton', planet_name: 'Hurston', space_station_name: 'Hidden', is_visible: 0 }
  ];
  const out = normalizeLocations(raw);
  const names = out.map((l) => l.name);
  assert.deepStrictEqual(names, ['ARC-L1 Wide Forest Station', 'Area 18', 'Rustville']);
